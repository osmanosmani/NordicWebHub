# NordicWebHub Documentation

This folder contains project documentation for NordicWebHub.

## Files

- [Screenshot checklist](./screenshot-checklist.md)
- [Project overview](./project-overview.md)
- [Features](./features.md)
- [Architecture](./architecture.md)
- [Local setup](./local-setup.md)
- [Deployment notes](./deployment-notes.md)
- [Database](./database.md)
- [Authentication and security](./authentication-and-security.md)
- [Security checklist](./security-checklist.md)
- [Project review checklist](./project-review-checklist.md)
- [Testing](./testing.md)
- [Project status](./project-status.md)
- [Future improvements](./future-improvements.md)
- [Final presentation material](./presentation.md)

## Screenshot Folder

Save final screenshots in:

```text
docs/screenshots/
```

See [Screenshot checklist](./screenshot-checklist.md) for the recommended list and naming convention.
